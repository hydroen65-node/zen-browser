...
class BrowserConcept {
  ...
  async init() {
    ...
    this.root.append(this.trigger, this.shelf, this.address);
  ...
  handleEvent(event) {
    ...
    if (
      ...
      if (event.type !== "TabAttrModified") this.panel.hidePopup();
      this.render();
      return;
    }
  ...
  addDock() {
    ...
    this.dock.append(
      this.node("span", "concept-dock-title", "Agents"),
      this.button("+", "New agent", (event) => {
  ...
  updateAddress() {
    ...
    this.address.textContent = (tab?.label || "New Tab").slice(0, 55);
  ...
  renderShelf() {
    ...
    top.append(
      search,
      this.button("+", "Pin current tab", () =>
        this.pinURL(
          this.win.gBrowser.selectedBrowser.currentURI.spec,
          this.win.gBrowser.selectedTab.label,
        ),
      ),
      this.button("▱", "New folder", () => this.createFolder()),
      this.button("×", "Close notch", () => this.toggle(false)),
    );
    ...
    if (this.path.length) {
      ...
      nav.append(
        this.button("⌂ " + (active?.name || "Home"), "Space home", () => {
          this.path = [];
          this.renderShelf();
        }),
      );
      ...
      this.path.forEach((id, i) => {
        ...
        if (!folder) return;
        nav.append(
          this.node("span", "concept-chevron", "›"),
      ...
      for (const space of this.win.gZenWorkspaces.getWorkspaces()) {
        const raw = space.icon;
        const icon =
          raw && !raw.endsWith(".svg") ? raw : space.name?.slice(0, 1) || "⌂";
        const b = this.button(
          icon,
          space.name,
          async () => {
    ...
    if (!cards.children.length)
      cards.append(this.node("span", "concept-empty", "No pinned items"));
  ...
  showCursorPrompt(data, browser) {
    ...
    if (data.kind === "selection") {
      ...
      const glow = this.node("span", "concept-task-light");
      const input = this.node("input");
      input.placeholder = "Ask about this page…";
      input.setAttribute("aria-label", "Agent prompt");
      const send = this.button("↑", "Open agent conversation", () => {});
      send.type = "submit";
    ...
    const height = data.kind === "selection" ? 42 : 60;
    const x = data.x - this.win.mozInnerScreenX;
    const y = data.y - this.win.mozInnerScreenY;
    this.panel.style.left =
      Math.max(8, Math.min(x - 24, this.win.innerWidth - 340)) + "px";
    this.panel.style.top =
      Math.max(8, Math.min(y - height - 8, this.win.innerHeight - height - 8)) +
      "px";
  ...
  showAgentPanel(prompt, context = {}, existingAgent = null) {
    ...
    header.append(
      this.node("span", "concept-task-light"),
      this.node("strong", "", "New agent"),
      this.button("×", "Close agent", () => {
        this.agentPanel.hidden = true;
      }),
    );
    const source = this.node(
      "div",
      "concept-agent-source",
      context.title || this.win.gBrowser.selectedTab.label,
    );
    ...
    const save = this.button(
      ...
      async () => {
        ...
        else this.space.agents.push(draft);
    ...
    this.agentPanel.append(input, persistent, save, message);
    input.focus();
  }
  renderAgents() {
