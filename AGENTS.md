...
- For the separate full-source app, run `npm run build:ui`, then `cd engine && ./mach package`, then `python3 scripts/build-concept-source.py` from the repository root. The builder requires Mozilla’s packaged `dist/zen/Nightly.app`; copying raw `dist/Nightly.app` caused localization failures. Verify native labels and navigation in `.concept-source-profile` before recommending it.
