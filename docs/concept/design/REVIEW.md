...
The next native review found the agent panel's narrow-window overlay left a thin, unreadable strip of the site beside it. Below 760px, the panel now occupies the content area and leaves the sidebar available. A source-build re-render caught a slight overlap with the sidebar's trailing controls; the final source render (`native-narrow-agent-source-2026-09-23.jpg`) shows a clean boundary. Stop is hidden when idle and appears only while a real page read is in progress. Consent denial and completed read states were checked natively, while the short-lived Stop appearance still needs a longer-running native task to inspect.

The next panel review exercised the local run protocol on the same MDN page. The activity list accurately separated a declined permission request from a completed read and kept the page content behind a collapsed details row. Its character count wrapped awkwardly, so the event copy was shortened. The final source re-render (`native-agent-run-source-2026-09-23.jpg`) keeps the count on one line.
