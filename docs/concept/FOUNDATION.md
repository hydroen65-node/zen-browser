...
- Offline tab-ownership state model. Active-tab requests remain pending until matching permission; another agent or space cannot steal a claim. This is a foundation, not an integrated execution gate yet.
...
2. Connect ownership to every actual tool action and to user interruption, tab navigation, tab closure, workspace changes and agent cancellation. Add visible takeover permission and immediate reclaim. Permission state must live in the trusted browser, never the model.
